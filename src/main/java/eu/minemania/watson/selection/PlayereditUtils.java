...
public class PlayereditUtils
{
    ...
    {
        Block block = BuiltInRegistries.BLOCK.getValue(Identifier.tryParse(blocks));
        if (block != Blocks.AIR)
        {
        ...
        {
            Item item = BuiltInRegistries.ITEM.getValue(Identifier.tryParse(blocks));
            if (item != Items.AIR)
            {
                return new ItemStack(item);
            }
            Optional<EntityType<?>> entityType = EntityType.byString(blocks);
            if (entityType.isPresent())
            {
                ...
                else if (BuiltInRegistries.ENTITY_TYPE.wrapAsHolder(entity).is(EntityTypeTags.BOAT))
                {
                ...
                else
                {
                    ...
                    {
                        return SpawnEggItem.byId(EntityType.VILLAGER)
                                .map(holder -> new ItemStack(holder.value()))
                                .orElse(new ItemStack(Items.BEDROCK));
                    }
                    var spawnEgg = SpawnEggItem.byId(entity);
                    if (spawnEgg.isEmpty())
                    {
                        InfoUtils.showGuiMessage(MessageType.WARNING, "watson.error.entity.not_found", entity.getDescription().getString());
                    ...
                    return new ItemStack(spawnEgg.get().value());
                }
            }
        }
