...
public class Analysis implements IChatHandler
{
    ...
    public boolean dispatchMatchedChat(MutableComponent chat)
    {
    ...
    {
        for (IMatchedChatHandler handler : m.get(pattern.getStringValue()))
        {
            m.put(pattern.getStringValue(), handler);
        }
        m.removeAll(pattern.getStringValue());
    }
